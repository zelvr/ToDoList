...
namespace ToDoList.Pages
{
    public class EditActivityModel : PageModel
    {
        ...
        public EditActivityModel(IDatabaseService<Activity> db, UserManager<IdentityUser> user, IFileService fileservice, ILogger<EditActivityModel> logger, TelegramNotifier notifier)
        {
            _db = db;
            _userManager = user;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (_ActivityFile != null)
            {
                ...
                _Activity.ActivityFile = await _db.Query().Where(a => a.Id == _Activity.Id).Select(a => a.ActivityFile).FirstOrDefaultAsync();
